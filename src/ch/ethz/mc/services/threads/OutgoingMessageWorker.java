...
public class OutgoingMessageWorker extends Thread {
	...
	public void run() {
		...
		try {
			TimeUnit.SECONDS
			.sleep(simulatorActive ? ImplementationConstants.MESSAGE_SENDING_CHECK_SLEEP_CYCLE_IN_SECONDS_WITH_SIMULATOR
					: ImplementationConstants.MESSAGE_SENDING_CHECK_SLEEP_CYCLE_IN_SECONDS_WITHOUT_SIMULATOR);
		...
		try {
			interventionExecutionManagerService.stopMessageSending();
		} catch (final Exception e) {
			log.warn("Error at stopping message sending: {}", e.getMessage());
		}
	}
}