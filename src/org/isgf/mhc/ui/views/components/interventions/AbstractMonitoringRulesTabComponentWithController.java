...
/**
 * Extends the abstract monitoring rules tab with a controller
 * 
 * @author Andreas Filler
 */
@SuppressWarnings("serial")
@Log4j2
public class AbstractMonitoringRulesTabComponentWithController extends
		AbstractMonitoringRulesTabComponent {
	...
	public AbstractMonitoringRulesTabComponentWithController() {
		super();
	...
	private void init(final Intervention intervention,
		...
		if (relatedMonitoringRuleId != null) {
			...
			getMonitoringRulesEditComponent().getMonitoringRuleInfoLabel()
					.setVisible(false);
			getMonitoringRulesEditComponent().getRulesTreePanelLayout()
	...
	public void editRule() {
		...
		String name = selectedAbstractMonitoringRule.getRuleWithPlaceholders();
		if (name == null || name.equals("")) {
			name = ImplementationContants.DEFAULT_OBJECT_NAME;
		}

		ThemeResource icon;
		if (selectedAbstractMonitoringRule.isSendMessageIfTrue()) {
	...
	private static class TreeSortDropHandler implements DropHandler {
		...
		private final AbstractMonitoringRulesTabComponentWithController	component;
		...
		public TreeSortDropHandler(
				...
				final AbstractMonitoringRulesTabComponentWithController component) {
			this.tree = tree;
